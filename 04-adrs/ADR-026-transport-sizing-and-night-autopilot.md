...
## Decision
1. **The fleet is scaled by passengers, the type is a land-train (trackless tram, ≈72 seats)**, not a 12-seat shuttle. The **MVP fleet is manned electric** (off-the-shelf, leased); the passenger-flow sizing below already applies from the **MIN (5000) MVP scenario** (autonomy is added later, in R3 — it does not change the sizing). Model: `boardings/day = visits × transport_share × 2`; peak/hour = day/10h × 1.5. Consist throughput ≈ 72 × 3 cycles/h × 0.8 ≈ ≈170 people/h; round-trip ≈20 min.
